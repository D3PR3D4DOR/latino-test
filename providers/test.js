    ...
    for (const embedUrl of embeddedUrls) {
      if (embedUrl.includes("embed69.org")) {
        ...
        for (const item of embed69Results) {
          ...
          // RESOLUCIÓN A URL DIRECTA DE VIDEO (REEMPLAZADO POR NUEVO RESOLVER MODULAR)
...
// ============================================================================
// FUNCIONES ADICIONALES: RESOLVER DE REPRODUCTORES MODULAR (COMPATIBLE QUICKJS)
// ============================================================================

/**
 * Algoritmo puro de unpacking para scripts Dean Edwards (p,a,c,k,e,d)
 */
...
/**
 * Resolutor específico para servidores con JS ofuscado (Medixiru, VidHide, StreamWish, Fastream)
 */
...
/**
 * Router Modular para resolver cualquier embed a su stream reproducible
 */
async function resolveEmbedToDirectStreamModular(embedUrl) {
  if (!embedUrl) return null;

  try {
    // Intentar primero con la función original de la base
    const directUrlOriginal = await resolveEmbedToDirectStream(embedUrl);
    if (directUrlOriginal) return directUrlOriginal;

    // Si la base no extrajo el .m3u8, evaluar por dominio específico
    ...
    // Intento genérico de desempaquetado de respaldo
    return await resolvePackedEmbedQuickJS(embedUrl);
  } catch (e) {
    return null;
  }
